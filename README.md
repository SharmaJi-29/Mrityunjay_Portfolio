# 🌌 MRITYUNJAY SHARMA  
### 🚀 Data Science | Machine Learning | AI Systems Builder  

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?size=28&duration=3000&color=00F7FF&center=true&vCenter=true&width=750&lines=AI+Engineer+in+the+Making;Machine+Learning+%7C+Computer+Vision+%7C+NLP;Building+Secure+Intelligent+Systems;Turning+Ideas+into+Real-World+AI+Solutions" />
</p>

---

## 🧠 About Me  

💡 A passionate **Computer Science student** with hands-on experience across **Python, Data Science, and Machine Learning**, driven by curiosity to build **real-world AI-powered systems**.  

⚡ I specialize in combining **analytical thinking + AI techniques** to develop **secure, scalable, and intelligent solutions**, especially in domains like:  
- 🔐 IoT Security  
- 🤖 Intelligent Automation  
- 🧠 AI-powered Education Systems

🎯 Continuously learning and exploring advanced topics like:  
  - Generative AI  
  - Transformer models  
  - AI for Security & Surveillance  

🎯 My mission:  
> Build impactful AI systems that solve real-world problems and create meaningful innovation.

---

## 📬 Contact Information  

<p align="center">
  <a href="mailto:sharmamrityunjay78@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  <a href="https://linkedin.com/in/yourlinkedin">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="https://github.com/yourusername">
    <img src="https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github"/>
  </a>
</p>

📍 Bhilai, Chhattisgarh, India  
📞 +91 9098218859  

---

## 🛠 Tech Stack  

### 💻 Programming  
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=for-the-badge)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge)

### 🤖 AI & Data  
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-FF6F00?style=for-the-badge)
![Deep Learning](https://img.shields.io/badge/Deep%20Learning-8A2BE2?style=for-the-badge)
![Data Science](https://img.shields.io/badge/Data%20Science-4CAF50?style=for-the-badge)
![Data Analytics](https://img.shields.io/badge/Data%20Analytics-20B2AA?style=for-the-badge)

### 👁️ Computer Vision & AI Domains  
![Computer Vision](https://img.shields.io/badge/Computer%20Vision-FF1493?style=for-the-badge)
![Neural Networks](https://img.shields.io/badge/Neural%20Networks-8B0000?style=for-the-badge)
![NLP](https://img.shields.io/badge/NLP-00CED1?style=for-the-badge)
![RAG](https://img.shields.io/badge/RAG%20Systems-FF4500?style=for-the-badge)

### 🧩 Core CS  
![Data Structures](https://img.shields.io/badge/Data%20Structures-000000?style=for-the-badge)
![DBMS](https://img.shields.io/badge/DBMS-4169E1?style=for-the-badge)
![Artificial Intelligence](https://img.shields.io/badge/Artificial%20Intelligence-9400D3?style=for-the-badge)

---

## 💼 Experience  

### 🔐 Legal Research Intern | IoT Security Research  
📍 Guru Ghasidas University, Bilaspur  
🗓️ Jan 2025 – Jun 2025  
- 🔍 Conducted in-depth research on **AI-based IoT security frameworks**  
- 🧠 Explored integration of **emotion detection systems** for identifying suspicious human behavior  
- 🔐 Studied how AI can enhance **intrusion detection and surveillance systems**  
- 📄 Contributed to research documentation and implementation concepts for **secure IoT environments**  
- 🚀 Gained exposure to **AI + Cybersecurity + Research methodologies**   

---

### 🐍 Python Programming Intern  
📍 MotionCut (AICTE)  
🗓️ Sep 2024 – Oct 2024  
- 💻 Developed multiple **Python-based applications and problem-solving scripts**  
- ⚡ Strengthened understanding of **algorithms, logic building, and clean coding practices**  
- 🧩 Solved real-world computational problems using **efficient Python solutions**   

---

### 📊 Data Science Intern  
📍 Oasis Infobyte  
🗓️ Aug 2024 – Sep 2024  
- 📈 Built **machine learning models** for classification and prediction tasks  
- 🧹 Performed **data cleaning, preprocessing, and feature selection**  
- 📊 Applied **EDA (Exploratory Data Analysis)** to uncover insights  
- 🎯 Improved model accuracy through **hyperparameter tuning and evaluation metrics**  
- 🧠 Gained hands-on experience with **real-world datasets and ML workflows**   

---

### 📈 Data Analysis Intern  
📍 Cognifyz  
🗓️ Jul 2024 – Aug 2024  
- 📊 Conducted **data preprocessing and statistical analysis** on structured datasets  
- 📉 Created **visualizations (charts, graphs, dashboards)** for better insights  
- 🔍 Identified trends and patterns to support **data-driven decision making**  
- ⚙️ Worked with tools and techniques for **data transformation and reporting** 

---

### 🤖 Data Science Intern  
📍 CodSoft  
🗓️ Jun 2024 – Jul 2024  
- 🤖 Developed **machine learning models using real-world datasets**  
- 📊 Implemented **classification and regression algorithms**  
- 🧪 Evaluated model performance using **accuracy, precision, recall, etc.**  
- 🔄 Understood complete ML lifecycle from **data → model → evaluation**   

---

## 🚀 Projects & Research  

### 🧠 AI-Powered Academic Assistant (RAG Based)  
🗓️ Sep 2025 – Jan 2026  
- Built an **AI learning assistant using RAG + NLP models**  
- Features:
  - 📚 AI-generated summaries  
  - 🤖 Intelligent chatbot  
  - 📅 Personalized study planner  
  - 🖼️ Visual question solving (OCR + AI)  

---

### 🔐 Enhancing IoT Security Through Emotion Detection  
🗓️ Jan 2025 – Jul 2025  
- 🔍 Designed an AI system to detect **human emotions for anomaly and threat identification**  
- 🔐 Applied **computer vision and deep learning** for real-time surveillance analysis  
- ⚙️ Proposed a secure framework combining **AI + IoT for intelligent intrusion prevention**   

---

### 📝 Automatic Answer Checker  
🗓️ Jul 2024 – Dec 2024  
- 🖼️ Built an **OCR-based system** to evaluate handwritten answer sheets automatically  
- 🧠 Used **NLP techniques** to compare answers and generate scoring logic  
- ⚡ Reduced manual evaluation effort by creating a **fast, scalable AI grading system** 

---

## 🎤 Conferences & Publications  

### 🏆 ICMSCI-2026 | University of Delhi  
🗓️ Feb 22, 2026  

- Presented research paper:  
  **“AI-Powered Academic Assistant”**  
- Demonstrated real-world applications of **AI in education systems**  
- Showcased innovation in **intelligent learning technologies**
- Gained recognition for innovation and practical implementation of AI

---

## 🎓 Education  

🎓 **B.Tech (Hons) – Computer Science & Technology**  
UTD-CSVTU, Bhilai (2022 – Present)  
📊 CGPA: 8.5  

📘 **Class XII (CBSE – Science)**  
JNV Bilaspur – 82.6%  

📗 **Class X (CBSE)**  
JNV Bilaspur – 77.4%  

---

## 🏆 Certifications  

- ☁️ Oracle Cloud – Data Science Professional Certification  
- 👁️ NPTEL – Computer Vision  
- 🧠 Internshala – Machine Learning (**Top Performer – 92%**)  
- 📊 Cisco – Data Analytics Essentials  
- 💼 Industry Simulations – BCG, Accenture, Tata, Goldman Sachs  
- 🐍 HackerRank – Python  

---

## 🚀 Key Strengths (Bonus Section – Adds Weightage)

- 🧠 Strong foundation in **AI, ML, and Data Science concepts**  
- 🔐 Unique combination of **AI + Security + IoT research**  
- 📊 Experience with **real-world datasets and industry-level problems**  
- 🛠 Ability to **build end-to-end AI solutions**  
- 🎤 Experience in **research presentation and technical communication**  
- 🚀 Fast learner with a mindset of **continuous improvement and innovation**  

---

## 🎯 Career Vision  

🚀 To become an **AI Engineer/Data Scientist** building:  
- Intelligent automation systems  
- Secure AI-driven infrastructures  
- Next-generation learning technologies  

---

## 🤝 Let's Build Something Amazing  

If you're interested in collaboration, internships, or AI research:  

📩 **Let’s connect and create impact together!**

---
